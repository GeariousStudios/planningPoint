...
export const fetchContent = async ({
...
}): Promise<{ items: ProductGroupItem[]; total: number; counts?: any }> => {
  ...
  const response = await fetch(`${apiUrl}/product-group?${params}`, {
    headers: {
...
export const deleteContent = async (id: number): Promise<void> => {
  const response = await fetch(`${apiUrl}/product-group/delete/${id}`, {
    method: "DELETE",
    headers: {
      "X-User-Language": localStorage.getItem("language") || "sv",
      "Content-Type": "application/json",
