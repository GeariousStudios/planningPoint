...
import { ProductFilters, ProductItem } from "../../types/manageTypes";
...
export const fetchContent = async ({
...
}): Promise<{ items: ProductItem[]; total: number; counts?: any }> => {
  ...
  if (filters?.masterPlanFieldIds) {
    for (const id of filters.masterPlanFieldIds) {
      params.append("masterPlanFieldIds", id.toString());
    }
  }
  // --- FILTERS STOP ---

  const response = await fetch(`${apiUrl}/product?${params}`, {
    headers: {
