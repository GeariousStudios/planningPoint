...
import { PlannedStopFilters, PlannedStopItem } from "../../types/manageTypes";
...
export const fetchContent = async ({
  ...
  sortOrder: SortOrder;
  search: string;
  filters?: PlannedStopFilters;
}): Promise<{ items: PlannedStopItem[]; total: number; counts?: any }> => {
  ...
  const response = await fetch(`${apiUrl}/planned-stop?${params}`, {
    headers: {
...
export const deleteContent = async (id: number): Promise<void> => {
  const response = await fetch(`${apiUrl}/planned-stop/delete/${id}`, {
    method: "DELETE",
    headers: {
      "X-User-Language": localStorage.getItem("language") || "sv",
      "Content-Type": "application/json",
